...
public class AgentProvider extends ProviderConfig {
    ...
    public void init(String providerName,
        ...
        try {
            AMIdentity provider =
                    new AMIdentity(token, providerName, IdType.AGENT, realm, null);
        ...
        try {
            ...
            if (profilePresent) {
                ...
                AMIdentity id = new AMIdentity(token,
                        providerName, IdType.AGENT, realm, null);
                if (debug.messageEnabled()) {
    ...
    /**
     * Obtain the name all registered WSP Agent profiles in the given realm.
     * 
     * @param providerType The type of provider to retrieve
     * @param realm The realm in which to search
     * @return A list of all names of registered provider agents
     * @throws ProviderException 
     */
    public static List<String> getAllProfileNames(String providerType, String realm) throws ProviderException {
        ...
        try {
            ...
            Set returnAttrSet = new HashSet();
            returnAttrSet.add(WSP_ENDPOINT);

            IdSearchResults results = idRepo.searchIdentities(IdType.AGENT, "*", control);
            Set agents = results.getSearchResults();
            if (!agents.isEmpty()) {
                for (Object a : agents) {
                    AMIdentity provider = (AMIdentity) a;
                    endpoints.add(provider.getName());
                }
            }
